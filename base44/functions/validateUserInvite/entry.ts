...
// Public endpoint — validates a user-invite token and returns the invited
// email so the register page can pre-fill / lock it.
Deno.serve(async (req) => {
  try {
    ...
    if (invite.status === 'completed') {
      return Response.json({ valid: false, reason: 'already_used', email: invite.email });
    }
