...
Deno.serve(async (req) => {
  ...
  if (data?.data) console.log('[getOSMBadgeDetails] data sub-keys:', Object.keys(data.data));
  const reqCount = Array.isArray(data?.data?.requirements) ? data.data.requirements.length : 0;
  console.log('[getOSMBadgeDetails] requirements count:', reqCount);
  if (reqCount > 0) console.log('[getOSMBadgeDetails] sample req:', JSON.stringify(data.data.requirements[0]));
