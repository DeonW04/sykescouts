...
// Scheduled job 2 — runs daily
// Finds events tomorrow with unpaid attending members and sends final reminder
  ...
  for (const event of targetEvents) {
    ...
    for (const action of actions) {
      ...
      for (const resp of attending) {
        ...
        const notifBody = `Final reminder: Payment of ${costStr} for ${event.title} is due tomorrow.`;
        const emailBody = `${notifBody}\n\nPlease log in to the SykeScouts app to complete payment before the event.\n\n40th Rochdale (Syke) Scouts`;
        ...
        for (const email of emails) {
          ...
          for (const sub of subs) {
            await base44.asServiceRole.functions.invoke('sendPushNotification', { subscription: sub.subscription_data, title: 'Final Payment Reminder', body: notifBody }).catch(() => {});
          }
          await base44.asServiceRole.integrations.Core.SendEmail({ to: email, subject: `Final payment reminder: ${event.title}`, body: emailBody }).catch(() => {});
        }
        sent++;
      }
    }
