...
export default function EventUpdateList() {
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const [searchTerm, setSearchTerm] = useState('');

  const { data: pages = [] } = useQuery({
    queryKey: ['communication-pages-event'],
    queryFn: () => base44.entities.CommunicationPage.filter({ type: 'event_update' }),
  });
  ...
  const deletePageMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['communication-pages-event'] });
      toast.success('Event update deleted');
    },
  });
  ...
  const createNewPage = async () => {
    ...
    navigate(createPageUrl('EventUpdate') + `?pageId=${pageId}`);
  };
      ...
      <div className="bg-gradient-to-r from-orange-500 to-red-600 text-white py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between">
            <h1 className="text-3xl font-bold">Event Updates</h1>
            <Button onClick={createNewPage} className="bg-white text-orange-600 hover:bg-gray-100">
        ...
        <Card className="mb-6">
          <CardContent className="p-4">
            <div className="relative">
              ...
              <Input
                placeholder="Search by title, date (YYYY-MM-DD), or month..."
                value={searchTerm}
            ...
            <Card>
              <CardContent className="p-12 text-center">
                <p className="text-gray-600 mb-4">
                  {pages.length === 0 ? 'No event updates created yet.' : 'No updates match your search.'}
                </p>
                {pages.length === 0 && (
                  <Button onClick={createNewPage} className="bg-orange-600 hover:bg-orange-700">
                    <Plus className="w-4 h-4 mr-2" />
                    Create First Update
                  </Button>
                )}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => navigate(createPageUrl('EventUpdate') + `?pageId=${page.page_id}`)}
                      >
