...
export default function EventUpdateList() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (newPage) => {
      ...
      navigate(createPageUrl('EventUpdate') + `?pageId=${newPage.page_id}`);
    },
  });
  ...
  const shareUrl = (pageId) => `${window.location.origin}/view/event-update/${pageId}`;
        ...
        {filteredPages.length === 0 ? (
          <Card>
            <CardContent className="p-12 text-center">
              <p className="text-gray-600 mb-4">No event updates yet</p>
              <Button
                onClick={() => createMutation.mutate()}
