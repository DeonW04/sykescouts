...
export default function MonthlyNewsletterList() {
  ...
  const { data: pages = [] } = useQuery({
    queryKey: ['communication-pages-monthly'],
  ...
  const deletePageMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['communication-pages-monthly'] });
  ...
  const createNewPage = async () => {
    ...
    navigate(createPageUrl('MonthlyNewsletter') + `?pageId=${pageId}`);
  };
            ...
            <Card>
              <CardContent className="p-12 text-center">
                <p className="text-gray-600 mb-4">
                  {pages.length === 0 ? 'No newsletters created yet.' : 'No newsletters match your search.'}
                </p>
                {pages.length === 0 && (
                  <Button onClick={createNewPage} className="bg-purple-600 hover:bg-purple-700">
                    <Plus className="w-4 h-4 mr-2" />
