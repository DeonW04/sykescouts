...
export default function TreasurerEventFinances() {
  ...
  const { data: events = [] } = useQuery({
    queryKey: ['events-with-cost'],
    queryFn: async () => {
      const all = await base44.entities.Event.list('-start_date', 100);
      return all.filter(e => (e.cost || 0) > 0);
    },
  });
  ...
  const paidCount = paymentStatuses.filter(ps => ps.event_id === eventId && ps.status === 'paid' && attendingMemberIds.includes(ps.member_id)).length;
  const totalCollected = ledgerEntries.filter(e => e.type === 'income').reduce((s, e) => s + (e.amount || 0), 0);
  const totalExpenses = (event?.estimated_expenses || []).reduce((s, e) => s + (e.amount || 0), 0);
  ...
  const unpaidMembers = attendingMemberIds
    .map(id => {
      ...
      const isOverdue = event?.payment_deadline && todayStr > event.payment_deadline;
  ...
  const sendReminder = async (memberId) => {
    ...
    for (const email of emails) {
      await base44.integrations.Core.SendEmail({ to: email, subject: `Payment reminder: ${event?.title}`, body: `Reminder: Payment of ${fmt(event?.cost)} for ${event?.title} is outstanding. Please log in to the parent portal to pay.` });
    }
    try {
      if (member.parent_one_email) await base44.functions.invoke('sendPushNotification', { email: member.parent_one_email, title: 'Payment reminder', body: `Payment of ${fmt(event?.cost)} for ${event?.title} is outstanding.` });
    } catch {}
  ...
  return (
    <TreasurerLayout title="Event Finances">
      ...
      {!event ? (
        <Card><CardContent className="py-12 text-center text-gray-500">No events with a cost found.</CardContent></Card>
      ) : (
        <div className="space-y-6">
          {/* Summary */}
          <Card>
            <CardHeader><CardTitle className="text-base">{event.title} — Summary</CardTitle></CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                <div><p className="text-xs text-gray-500">Date</p><p className="font-semibold">{format(new Date(event.start_date), 'd MMM yyyy')}</p></div>
                <div><p className="text-xs text-gray-500">Cost / member</p><p className="font-semibold">{fmt(event.cost)}</p></div>
                ...
                <div className="col-span-2">
                  <p className="text-xs text-gray-500">Surplus / Shortfall</p>
                  <p className={`font-bold text-lg ${surplus >= 0 ? 'text-green-600' : 'text-red-600'}`}>{surplus >= 0 ? '+' : ''}{fmt(surplus)}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          {/* Payments Table */}
          <Card>
            <CardHeader><CardTitle className="text-base">Payments — {event.title}</CardTitle></CardHeader>
            <CardContent>
              {ledgerEntries.length === 0 ? (
                <p className="text-sm text-gray-500 text-center py-6">No ledger entries for this event yet.</p>
              ) : (
