...
export default function WeeklyMessageList() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (newPage) => {
      ...
      navigate(createPageUrl('WeeklyMessage') + `?pageId=${newPage.page_id}`);
    },
  });
  ...
  const shareUrl = (pageId) => `${window.location.origin}/view/weekly-message/${pageId}`;
        ...
        {filteredPages.length === 0 ? (
          <Card>
            <CardContent className="p-12 text-center">
              <p className="text-gray-600 mb-4">No weekly messages yet</p>
              <Button
                onClick={() => createMutation.mutate()}
