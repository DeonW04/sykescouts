...
import { ChevronDown, ChevronRight, Calendar, CheckCircle, AlertTriangle, XCircle } from 'lucide-react';
...
export default function TreasurerProgrammeFinances() {
  ...
  // Meetings in term for this section
  ...
  // Calc expenses: receipt allocations + ledger expenses linked to meetings in this term
  const calcExpenses = useMemo(() => {
    const progIds = new Set(termMeetings.map(p => p.id));
    const receiptSpend = allocations.filter(a => a.linked_meeting_id && progIds.has(a.linked_meeting_id))
      .reduce((s, a) => s + (a.amount || 0), 0);
    const ledgerSpend = ledgerEntries.filter(e => e.type === 'expense' && e.section_id === activeSection?.id && activeTerm && e.date >= activeTerm.start_date && e.date <= activeTerm.end_date)
      .reduce((s, e) => s + (e.amount || 0), 0);
    return receiptSpend + ledgerSpend;
  }, [termMeetings, allocations, ledgerEntries, activeSection, activeTerm]);

  // Calc income: cost * members attending for meetings with cost
  const calcIncome = useMemo(() => {
    return termMeetings.reduce((total, mtg) => {
      ...
      let attendingCount;
      if (attendanceAction) {
        attendingCount = actionResponses.filter(r =>
          r.action_required_id === attendanceAction.id &&
          (r.response_value === 'Yes, attending' || r.response_value === 'yes')
        ).length;
      } else {
        attendingCount = sectionMembers.length;
      }
      return total + attendingCount * mtg.cost;
    }, 0);
  }, [termMeetings, actionsRequired, actionResponses, sectionMembers]);

  const remainingBudget = budgetAmount + calcIncome - calcExpenses;

  const getMeetingMemberPayments = (mtg) => {
    if (!mtg.cost) return [];
    const attendanceAction = actionsRequired.find(a => a.programme_id === mtg.id && a.action_purpose === 'attendance');
    ...
    return sectionMembers.map(member => {
      ...
      return { member, paid, attendingStatus, cost: mtg.cost };
    });
  };

  const StatusPill = ({ cost, paid, attendingStatus }) => {
    if (attendingStatus === 'not_attending') return (
      <Badge className="bg-gray-100 text-gray-600 text-xs">Not Attending</Badge>
    );
    if (paid === 0) return (
            ...
            <SelectContent>
              {terms.map(t => (
                <SelectItem key={t.id} value={t.id}>
                  {t.title || t.name}{currentTerm?.id === t.id ? ' ✓ Current' : ''}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
      ...
      {termMeetings.length === 0 ? (
        ...
        <div className="space-y-2">
          {termMeetings.map(mtg => {
            ...
            const memberData = hasCost ? getMeetingMemberPayments(mtg) : [];
                      ...
                      <div className="mt-3">
                        ...
                        <div className="overflow-x-auto">
                          <table className="w-full text-xs">
                            ...
                            <tbody>
                              {memberData.map(({ member, paid, attendingStatus, cost }) => (
                                <tr key={member.id} className="border-b border-gray-100">
                                  <td className="py-1.5 font-medium">{member.full_name}</td>
                                  <td className="py-1.5 text-right">{fmt(cost)}</td>
                                  <td className="py-1.5 text-right text-green-600">{fmt(paid)}</td>
                                  <td className="py-1.5 text-center">
                                    <StatusPill cost={cost} paid={paid} attendingStatus={attendingStatus} />
                                  </td>
                                </tr>
                              ))}
                            </tbody>
