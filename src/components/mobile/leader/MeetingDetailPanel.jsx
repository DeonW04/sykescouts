...
import { CheckCircle2, XCircle, Clock, FileText, ListChecks, Bell, Users, CreditCard, Plus, Trash2, ChevronDown, ChevronUp, Loader2 } from 'lucide-react';
...
export default function MeetingDetailPanel({ programme, sections, onClose }) {
  ...
  const handleRegisterById = async (member) => {
    ...
    setRegisterError('');
    try {
      const result = await base44.functions.invoke('verifyStripePaymentById', { payment_intent_id: paymentIntentInput.trim(), member_id: member.id });
      if (result.data?.error) throw new Error(result.data.error);
      const amount = result.data?.amount || programme.cost || 0;
      ...
      if (existing) {
        await base44.entities.MeetingPaymentStatus.update(existing.id, { status: 'paid', paid_at: todayStr, stripe_payment_intent_id: paymentIntentInput.trim(), card_brand: cardBrand, card_last4: cardLast4 });
      } else {
        await base44.entities.MeetingPaymentStatus.create({ meeting_id: programme.id, member_id: member.id, status: 'paid', paid_at: todayStr, stripe_payment_intent_id: paymentIntentInput.trim(), card_brand: cardBrand, card_last4: cardLast4 });
      }
      await refetchPS();
                            ...
                            <div className="pt-3 space-y-3">
                              {!isRegisterMode ? (
                                <div className="flex flex-col gap-2">
                                  ...
                                  <button onClick={() => { setRegisterByIdFor(member.id); setPaymentIntentInput(''); setRegisterError(''); }} className="w-full py-2.5 bg-[#004851] text-white rounded-xl text-sm font-semibold">Register payment by ID</button>
                                </div>
                              ) : (
                                <div className="space-y-2">
                                  <p className="text-xs font-semibold text-gray-600">Enter Stripe Payment Intent ID:</p>
                                  <input value={paymentIntentInput} onChange={e => { setPaymentIntentInput(e.target.value); setRegisterError(''); }} placeholder="pi_..." className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-mono focus:outline-none focus:border-[#004851]" />
                                  {registerError && <p className="text-xs text-red-500">{registerError}</p>}
                                  <div className="flex gap-2">
                                    <button onClick={() => { setRegisterByIdFor(null); setPaymentIntentInput(''); setRegisterError(''); }} className="flex-1 py-2.5 border border-gray-200 text-gray-600 rounded-xl text-sm font-semibold">Cancel</button>
                                    <button onClick={() => handleRegisterById(member)} disabled={!paymentIntentInput.trim() || registeringId} className="flex-1 py-2.5 bg-[#004851] text-white rounded-xl text-sm font-semibold disabled:opacity-50 flex items-center justify-center gap-2">
