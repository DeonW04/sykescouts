    ...
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-purple-900">
            <Image className="w-5 h-5" />
            Parent Dashboard Hero Images
          </CardTitle>
          <p className="text-sm text-gray-500">
            The hero banner shown at the top of the parent dashboard — changes based on which section the child is in.
          </p>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <FixedImageSlot
              key={slug}
              label={`${label} dashboard hero image`}
              hint={`Shown to parents of ${label}. Landscape photo recommended.`}
              image={getFixed('parent_dashboard', slug)}
              onUpload={(f) => handleFixedUpload('parent_dashboard', slug, f)}
              onDelete={handleFixedDelete}
              uploading={uploading}
              uploadId={`parent-dash-${slug}-upload`}
            />
          ))}
        </CardContent>
      </Card>
