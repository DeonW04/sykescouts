...
export default function PDFGenerator({ assessment }) {
  const generatePDF = () => {
...
<head>
  <meta charset="UTF-8">
  <title>Risk Assessment - ${assessment.activity_name}</title>
  <style>
    @page {
      size: A4 landscape;
      margin: 12mm;
...
<body>
  ...
  <table class="metadata">
    ...
    <tr>
      ...
      <td>${assessment.assessment_date ? format(new Date(assessment.assessment_date), 'dd/MM/yyyy') : ''}</td>
      <td class="label">Date of next review</td>
      <td colspan="2">${assessment.next_review_date ? format(new Date(assessment.next_review_date), 'dd/MM/yyyy') : ''}</td>
    </tr>
  </table>
